...
impl Config {
    pub fn new(store: luffa_store::config::Config, p2p: luffa_node::config::Config) -> Self {
        Self {
            store,
            p2p,
...
fn default_store_config(store_path: Option<PathBuf>) -> Result<luffa_store::config::Config> {
    let path = config_data_path(store_path)?;
    Ok(luffa_store::config::Config { path })
}

fn default_p2p_config(key_store_path: PathBuf) -> luffa_node::config::Config {
